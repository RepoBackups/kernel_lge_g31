...
unsigned int mpdecstate = MSM_MPDEC_IDLE;
...
static int mp_decision(void) {
	...
	int new_mpdecstate = MSM_MPDEC_IDLE;
	int nr_cpu_online;
	int index;
	...
	if (mpdecstate == MSM_MPDEC_DISABLED)
	...
	if (new_mpdecstate != MSM_MPDEC_IDLE) {
		total_time = 0;
	}

	last_time = ktime_to_ms(ktime_get());
#if DEBUG
	pr_info(MPDEC_TAG"[DEBUG] rq: %u, new_mpdecstate: %i | Mask=[%d%d%d%d]\n",
			rq_depth, new_mpdecstate, cpu_online(0), cpu_online(1), cpu_online(2), cpu_online(3));
#endif
	return new_mpdecstate;
}
...
static void msm_mpdec_work_thread(struct work_struct *work) {
	...
	case MSM_MPDEC_UP:
		...
		break;
	default:
		pr_err(MPDEC_TAG"%s: invalid mpdec hotplug mpdecstate %d\n",
			__func__, mpdecstate);
	}
	...
	if (per_cpu(msm_mpdec_cpudata, cpu).is_boosted) {
		if (mutex_trylock(&per_cpu(msm_mpdec_cpudata, cpu).unboost_mutex)) {
			...
			if (!cpu_policy) {
				...
				if (cpu_mod) {
					pr_info(MPDEC_TAG"cpu%i was modified. Restoring mpdecstate...", cpu);
					cpu_down(cpu);
				}
				return;
			}
